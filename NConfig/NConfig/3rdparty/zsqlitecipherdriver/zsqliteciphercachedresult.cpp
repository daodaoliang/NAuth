...
//This file is from %QtSrcCode%\qtbase\src\sql\kernel\qsqlcachedresult.cpp
//Modified by zq @20140410.

static const uint initial_cache_size = 128;

class ZSqliteCipherCachedResultPrivate
{
public:
    ZSqliteCipherCachedResultPrivate();
    ...
    ZSqliteCipherCachedResult::ValueCache cache;
    int rowCacheEnd;
    int colCount;
    bool forwardOnly;
    bool atEnd;
...
ZSqliteCipherCachedResultPrivate::ZSqliteCipherCachedResultPrivate():
    rowCacheEnd(0), colCount(0), forwardOnly(false), atEnd(false)
{
}

void ZSqliteCipherCachedResultPrivate::cleanup()
{
    cache.clear();
    forwardOnly = false;
    atEnd = false;
...
void ZSqliteCipherCachedResultPrivate::init(int count, bool fo)
{
    Q_ASSERT(count);
    cleanup();
    forwardOnly = fo;
...
int ZSqliteCipherCachedResultPrivate::nextIndex()
{
    if (forwardOnly)
        return 0;
...
bool ZSqliteCipherCachedResultPrivate::canSeek(int i) const
{
    if (forwardOnly || i < 0)
        return false;
...
void ZSqliteCipherCachedResultPrivate::revertLast()
{
    if (forwardOnly)
        return;
    rowCacheEnd -= colCount;
...
inline int ZSqliteCipherCachedResultPrivate::cacheCount() const
{
...
ZSqliteCipherCachedResult::ZSqliteCipherCachedResult(const QSqlDriver * db): QSqlResult (db)
{
    d = new ZSqliteCipherCachedResultPrivate();
}

ZSqliteCipherCachedResult::~ZSqliteCipherCachedResult()
{
    delete d;
}
...
bool ZSqliteCipherCachedResult::fetchPrevious()
{
    return fetch(at() - 1);
}
...
QVariant ZSqliteCipherCachedResult::data(int i)
{
...
bool ZSqliteCipherCachedResult::isNull(int i)
{
    int idx = d->forwardOnly ? i : at() * d->colCount + i;
    if (i >= d->colCount || i < 0 || at() < 0 || idx >= d->rowCacheEnd)
        return true;
...
void ZSqliteCipherCachedResult::cleanup()
{
...
void ZSqliteCipherCachedResult::clearValues()
{
    setAt(QSql::BeforeFirstRow);
    d->rowCacheEnd = 0;
    d->atEnd = false;
...
int ZSqliteCipherCachedResult::colCount() const
{
    return d->colCount;
}

ZSqliteCipherCachedResult::ValueCache &ZSqliteCipherCachedResult::cache()
{
    return d->cache;
}

void ZSqliteCipherCachedResult::virtual_hook(int id, void *data)
{
    QSqlResult::virtual_hook(id, data);
}

void ZSqliteCipherCachedResult::detachFromResultSet()
{
    cleanup();
}

void ZSqliteCipherCachedResult::setNumericalPrecisionPolicy(QSql::NumericalPrecisionPolicy policy)
{
